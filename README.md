...
### Память Qdrant: кластеры и атрибуты

При сохранении в память (`Сохранить в память` / `POST /api/v1/memory/save`) в Qdrant пишутся **название**, **значения атрибутов** и **имя кластера** из утверждённых кластеров. При следующей кластеризации позиции из памяти группируются по `cluster_name`, а не по одной вкладке на строку.

Точки, сохранённые до этого обновления (без `cluster_name`), попадают в fallback-кластер «Память». Чтобы восстановить группировку: пройдите цикл clusterize → normalize → **Сохранить в память**, либо очистите Qdrant и сохраните заново. Эмбеддинги при сохранении используют тот же провайдер, что выбран в UI (`local` / `gemini`).
